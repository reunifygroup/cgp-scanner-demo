...
-   **100% TypeScript** - no Python dependencies in production
...
Instead of traditional CNN classification (which requires retraining for new cards), this scanner uses **embedding-based recognition**:
...
-   Professional scanners (TCGPlayer, Delver Lens) use this approach
...
# Download card images (default: 50 cards)
...
\`\`\`bash
CARD_LIMIT=100 npm run download-cards
\`\`\`
...
**Performance**: ~13ms per image on modern hardware
...
tcgdex/
├── scripts/
│   ├── download-cards.ts           # TCGdex API downloader
│   ├── augment-dataset-advanced.ts # Image augmentation (TypeScript + Sharp)
│   └── generate-embeddings.ts      # Embedding extraction (MobileNet v2)
├── src/
│   ├── App.tsx                     # Main React scanner component
│   ├── App.css                     # Styles
...
├── training-data/                  # Augmented images (gitignored)
...
Edit \`vercel.json\` to control card limit:
...
-   50 cards: ~5-10 minutes
-   250 cards: ~10-15 minutes
-   1000 cards: ~25-35 minutes
...
### Option 1: Download from TCGdex

Edit \`scripts/download-cards.ts\`:

\`\`\`typescript
const sets = ["sv02", "sv03", "sv04"]; // Add more sets
\`\`\`

Then run:

\`\`\`bash
CARD_LIMIT=1000 npm run download-cards
\`\`\`

### Option 2: Add Custom Images

1. Add high-quality card images to \`images/\` directory
2. Filename format: \`{id}_{name}.png\` (e.g., \`sv02-185_Iono.png\`)
3. Run the pipeline:
    \`\`\`bash
    npm run pipeline
    \`\`\`
...
| **Model size**          | ~4MB (MobileNet v2)                               |
| **Inference time**      | ~50-100ms per frame                               |
| **Scanning frequency**  | 2 FPS (every 500ms)                               |
| **Embeddings DB size**  | ~2KB per card                                     |
| **Accuracy**            | High similarity scores (>80%) for correct matches |

## Roadmap

-   [ ] Support for multiple card sets
-   [ ] Confidence threshold filtering
-   [ ] Card metadata display (HP, attacks, price)
-   [ ] Bulk scanning mode
-   [ ] Export scan history
...
-   **No Python in production**: All scripts use TypeScript + TensorFlow.js
-   **Python scripts exist** for reference/legacy but are not used in deployment
-   **Embeddings are ephemeral**: Regenerated on each deploy (not committed to git)
-   **Scalability**: Can handle 20,000+ cards with same model size
