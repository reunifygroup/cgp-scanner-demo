...
const EMBEDDINGS_OUTPUT_DIR = "public/embeddings"; // where to save the results
...
async function loadAndPreprocessImage(imagePath: string): Promise<tf.Tensor3D> {
    ...
    const imageTensor = tf.tensor3d(new Uint8Array(imageBuffer.data), [IMAGE_SIZE, IMAGE_SIZE, 3], "int32");
 ...
 */
async function extractEmbedding(model: mobilenet.MobileNet, imagePath: string): Promise<number[]> {
...
async function generateEmbeddingsDatabase(): Promise<void> {
    ...
    for (let i = 0; i < imageFiles.length; i++) {
        ...
        try {
            ...
            if ((i + 1) % 10 === 0 || i === imageFiles.length - 1) {
                process.stdout.write(`\r   Processed ${i + 1}/${imageFiles.length} images...`);
            }
        } catch (error) {
    ...
    console.log(`   (${((elapsedTime / embeddingsData.length) * 1000).toFixed(1)}ms per image)`);
    ...
    await writeFile(embeddingsJsonPath, JSON.stringify(embeddingsDatabase, null, 2));
